...
main() {
    ...
    json_add "$FAILOVER_FILE" "timestamp" "$(date -Iseconds)"
    json_add "$FAILOVER_FILE" "mesh_ok" "$mesh_ok"

    if [[ "$mesh_ok" == "true" ]]; then
        log "PASS" "Mesh healthy — no failover required" "$GREEN"
        json_add "$FAILOVER_FILE" "failover_required" "false"
        json_close "$FAILOVER_FILE"
        exit 0
    fi

    log "WARN" "Mesh unhealthy or supervisor failed — failover required" "$YELLOW"
    json_add "$FAILOVER_FILE" "failover_required" "true"

    if echo "failover" | nc -u -w1 "$PEER_IP" "$PEER_PORT" &>/dev/null; then
        log "PASS" "Failover packet sent" "$GREEN"
        json_add "$FAILOVER_FILE" "failover_packet" "sent"
    else
        log "FAIL" "Failover packet failed" "$RED"
        json_add "$FAILOVER_FILE" "failover_packet" "failed"
    fi
